...
* Calibrated the transition path to the IMF program (Country Report 26/174, fifth ECF review, Tables 1, 2b and 4b) instead of holding FY2024/25 spending ratios fixed for twenty years. Previously the baseline ran a primary deficit of 5.6–6.1 percent of GDP for its first two decades (government consumption 5.8, transfers 4.0 and public investment 5.0 percent of GDP against 8.7 percent of revenue), debt climbed from 50 to 84 percent of GDP by year 10, and the closure rule later pushed government consumption below zero. The fiscal block now follows the program for FY2024/25–FY2030/31 (model periods 0–6) and holds the last value: government consumption (`alpha_G`, recurrent spending net of interest and cash transfers) 4.3 → 5.9 percent of GDP, cash transfers (`alpha_T`: fuel and fertilizer subsidies, the safety net, public pensions) 2.0 → 1.5 percent, public investment (`alpha_I`, capital expenditure) 5.0 → 5.1 percent, grants (`alpha_FA`, now a path) 1.7 → 0.3 percent, and revenue rising with the program from 9.2 to 12.3 percent of GDP through the consumption-tax rate (`tau_c` 0.060 → 0.092) and CIT collections (`adjustment_factor_for_cit_receipts` 0.327 → 0.525), two thirds and one third of the gain. `initial_debt_ratio` is 0.505 (the program's FY2024/25 figure) and the closure rule starts after the program (`tG1` 20 → 7). Reproducing the program's debt decline with primary balances near zero requires a negative real effective interest rate on the legacy debt — the counterpart of inflation and concessional terms eroding it — which the model cannot represent: OG-Core floors the sovereign rate at zero. `r_gov_shift` is therefore a path that holds the rate at the floor through the program years (the program-implied rate, −3.5 to −1.4 percent, is documented) and converges over four periods to a long-run 2 percent (the previous 4.5 percent long-run rate exceeded even the program's FY2030/31 nominal effective rate of 5.2 percent). The centering constant of the debt-elastic premium is now subtracted from the shift, as OG-Core's formula requires (`r_gov_DY d + r_gov_DY2 d² = r_gov_DY2 (d − D̄)² − r_gov_DY2 D̄²`); the previous calibration added it, which put the long-run sovereign rate about 0.7 percentage points below the level it documented. With the real rate floored at zero the model's debt ratio declines more slowly than the program's — to about 36 rather than 29 percent of GDP by FY2030/31 — and reaches the 30 percent target through the closure rule afterwards; the gap is the inflation-tax channel a real model leaves out.
* Calibrated household patience to Ethiopia's capital-output ratio: `beta_annual` 0.96 → 0.92 for all groups. The inherited US value put the steady-state capital-output ratio near 2.9 and gross investment above 40 percent of GDP, against the Penn World Table's 2.2 and the program's 20–28 percent; with the return on capital close to 1/β − 1 and K/Y = γ/(r + δ), matching 2.2 needs r ≈ 8.6 percent and β ≈ 0.92 (`households.md`). In the open economy the adjustment is partial — foreign capital enters at the world rate as domestic saving falls — so the steady state lands at r = 5.8 percent, K/Y = 2.6 and investment of 36 percent of GDP, a third of the way to the data; the remainder is the OG-Core saving block's family-wide tendency to over-accumulate capital and is documented as such. The paths are derived in `macro_params` from the IMF tables and the packaged `g_n`, rebuilt by `update_baseline.py`, and pinned by `tests/test_fiscal_program.py`. The steady-state initial guesses are retuned to the solved values (`initial_guess_r_SS` 0.052 → 0.044, `initial_guess_TR_SS` 0.057 → 0.009, `initial_guess_factor_SS` 139,355 → 323,000): the old transfer guess corresponded to transfers near 10 percent of GDP and the factor guess was less than half the solved value, and with transfers now 1.5 percent of GDP OG-Core's initial-guess sweep — which varies only the interest-rate guess — no longer found the steady state from them.
