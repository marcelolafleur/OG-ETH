...
* Calibrated the capital block to Ethiopia's capital-output ratio and foreign-owned capital stock. The inherited US patience (`beta_annual` 0.96) put the steady-state capital-output ratio near 2.9 and gross investment above 40 percent of GDP, against the Penn World Table's 2.2 and the program's 20–28 percent; and the 4 percent world rate — which in OG-Core fixes the capital stock foreign investors are willing to hold, `K_f = zeta_K (K_open − K_d)` — put the foreign-owned capital stock at 0.37 of GDP against a ~0.24 FDI stock. Two levers, both set to data: `beta_annual` 0.96 → 0.90 (household wealth near 2.2 times GDP, the same magnitude as the initial-wealth anchor) and `world_int_rate_annual` 0.04 → 0.063, UNCTAD's measured return on inward FDI in Africa (World Investment Report 2018). Patience alone (0.92 with the 4 percent world rate) had taken the capital-output ratio only to 2.6 while pushing the foreign-owned stock up; `households.md` and the world-rate section of `macro.md` document the arithmetic.
