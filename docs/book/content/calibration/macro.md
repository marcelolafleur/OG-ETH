...
evaluated at the program debt ratios $d_t$ for the program years and at $\bar{D}$ thereafter, with $\bar{r}$ the steady-state return on capital (the transition's $r$ moves by at most a percentage point, which the 0.245 scale turns into a few basis points of $r_{gov}$).  The path is built by `macro_params.r_gov_shift_path` and pinned by `tests/test_fiscal_program.py`.
