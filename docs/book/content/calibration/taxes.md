...
The same formality proxy decides who *receives* a public pension.  OG-Core pays every household a retirement benefit through its US-style social-security formula; in Ethiopia only formal employees covered by the two agencies are entitled to one, and they are the same households that remit income tax.  We set `replacement_rate_adjust` by lifetime-income group to [0, 0, 0, 0, 0, 0.5, 1] — no public pension for the five informal groups, half for the sixth, full for the top group — mirroring the compliance vector.  The payroll rate stays an economy-wide effective rate (OG-Core applies one rate to all labour income), so the incidence of contributions is spread while benefits are concentrated on the covered minority; the aggregate pension outlay and its financing are consistent with the covered share, which is what the fiscal block needs.
