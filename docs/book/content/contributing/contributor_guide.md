...
The purpose of this guide is to provide the necessary background such that you can make improvements to the `OG-ETH` and share them with others working on the model.

`OG-ETH` code is tracked by using [`Git`](https://help.github.com/articles/github-glossary/#git) version control software along with the [GitHub.com](https://github.com/) web platform for `Git` workflow and collaboration. Following the next steps will get you up and running and contributing to the model even if you've never used `Git` or other version control software.
...
4. Sign in to GitHub and create your own [remote](https://help.github.com/articles/github-glossary/#remote) [repository](https://help.github.com/articles/github-glossary/#repository) (repo) of `OG-ETH` by clicking [Fork](https://help.github.com/articles/github-glossary/#fork) in the upper right corner of the [OG-ETH GitHub repository page](https://github.com/EAPD-DRB/OG-ETH). Select your username when asked "Where should we fork this repository?"

5. From your command line, navigate to the directory on your computer where you would like your local repo to live.

6. Create a local repo by entering at the command line the text, shown in the code blocks below after the `$` symbol.[^commandline_note] This step creates a directory called `OG-ETH` in the directory that you specified in the prior step:

    ```
      $ git clone https://github.com/[github-username]/OG-ETH.git
    ```

7. From your command line or terminal, navigate to your local `OG-ETH` directory.
...
8. Make it easier to [push](https://help.github.com/articles/github-glossary/#pull) your local work to others and [pull](https://help.github.com/articles/github-glossary/#pull) others' work to your local machine by entering at the command line:
    ...
    ```
      $ cd OG-ETH
      OG-ETH$ git remote add upstream https://github.com/EAPD-DRB/OG-ETH.git
    ```

9. Create the project environment, with all of the necessary packages to execute the source code, in a single step:

    ```
      OG-ETH$ uv sync --extra dev
    ```

    This creates a local `.venv` directory with the `ogeth` package and its development dependencies installed at the exact versions pinned in `uv.lock` (downloading a compatible Python interpreter if needed). For documentation/Jupyter Book work, run `uv sync --extra dev --extra docs` instead.

10. Run commands in the environment by prefixing them with `uv run` (for example, `uv run python examples/run_og_eth.py`), or activate the environment first:

    ```
      OG-ETH$ source .venv/bin/activate
    ```

    (on Windows PowerShell: `.\.venv\Scripts\Activate.ps1`).

If you have made it this far, you've successfully made a remote copy (a
fork) of the central `OG-ETH` repo. That remote repo is hosted on GitHub.com at [https://github.com/EAPD-DRB/OG-ETH](https://github.com/EAPD-DRB/OG-ETH). You have also created a local repo (a [clone](https://help.github.com/articles/github-glossary/#clone)) that lives on your machine and only you can see; you will make your changes to
the OG-ETH model by editing the files in the `OG-ETH`
...
`OG-ETH`.  Different workflows may be necessary in some
situations, in which case other contributors are here to help.

1. Before you edit the `OG-ETH` source code on your machine,
   ...
   a. Tell Git to switch to the main branch in your local repo.
      Navigate to your local `OG-ETH` directory and enter the
      following text at the command line:

    ```
        OG-ETH$ git checkout main
    ```

   b. Download all of the content from the central `OG-ETH` repo:
    ```
        OG-ETH$ git fetch upstream
    ```
...
4. Now you're ready to [commit](https://help.github.com/articles/github-glossary/#commit) your changes to your local repo using the code below. The first line of code tells `Git` to track a file. Use "git status" to find all the files you've edited, and "git add" each of the files that you'd like `Git` to track. As a rule, do not add large files. If you'd like to add a file that is larger than 25 MB, please contact the other contributors and ask how to proceed. The second line of code commits your changes to your local repo and allows you to create a commit message; this should be a short description of your changes.
   ...
   *Tip*: Committing often is a good idea as `Git` keeps a record of your changes. This means that you can always revert to a previous version of your work if you need to. Do this to commit:
    ```
     OG-ETH$ git add [filename]
     OG-ETH$ git commit -m "[description-of-your-commit]"
    ```
...
6. When you are ready for other team members to review your code, make your final commit and push your local branch to your remote repo:
    ```
     OG-ETH$ git push origin [new-branch-name]
    ```
...
[^recent_python]:`OG-ETH` is currently tested to run on Python 3.12 and 3.13. You do not need to manage the Python version yourself: `uv sync` reads the project's requirements and downloads a compatible interpreter automatically.
